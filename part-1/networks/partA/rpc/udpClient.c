...
void checkReady()
{
    ...
    int bytesReceived = recvfrom(sockfd, buffer, 1024, 0, (struct sockaddr *)&addr, &addr_size);
    if (bytesReceived < 0)
    {
...
void acceptAndSendInput()
{
    ...
    bytesSent = sendto(sockfd, inputChoice, 1024, 0, (struct sockaddr *)&addr, sizeof(addr));
    if (bytesSent == -1)
    {
        perror("Error in sendto");
        exit(1);
