...
void scheduler(void)
{
  ...
  for (;;)
  {
    ...
    int min_ctime = ticks;
    // get the process with minimum ctime = creation time
    for (p = proc; p < &proc[NPROC]; p++)
    {
      acquire(&p->lock);
...
void procdump(void)
{
  ...
  for (p = &proc[3]; p < &proc[NPROC]; p++)
  {
    ...
    if (p->state >= 0 && p->state < NELEM(states) && states[p->state])
      ...
      state = "???";
    // printf("%d,%d,%d", ticks, p->pid,p->priority);
    printf("[%d] %d %s Q:%d E:%d R:%d W:%d", ticks, p->pid, state, p->priority, p->entryTimeInQueue, p->runTimeInQueue, p->waitTimeInQueue);
    printf("\n");
  }
}
